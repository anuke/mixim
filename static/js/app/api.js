    ...
    $.getJSON('/json/media/get/' + picId + '/', {},
        function (data) {
            if (data.success) {
                ...
                $('#code3').attr( "value", '<a href="' + photo.original + '"><img src="' + photo.original.replace('original', 'resized/598x598') + '"/></a>');
                $('#code4').attr( "value", '[img]' + photo.original + '[/img]');
                $('#code5').attr( "value", '[url=' + photo.original + '][img]' + photo.original.replace('original', 'resized/598x598') + '[/img][/url]');
                $('#code6').attr( "value", 'http://mixim.ru/pic' + photo.id );
                ...
                $("#share20_fb").attr("onclick", "window.open('" + fbUrl + "', 'share', 'height=440, width=540, status=no, toolbar=no, menubar=no, location=no');return false");
                $("#share20_ok").attr("onclick", "window.open('" + okUrl + "', 'share', 'height=440, width=540, status=no, toolbar=no, menubar=no, location=no');return false");
                /*
